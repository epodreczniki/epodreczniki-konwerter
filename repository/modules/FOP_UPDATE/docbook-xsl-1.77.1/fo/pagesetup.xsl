  ...
  <fo:block>
    ...
    <xsl:choose>
      ...
      <xsl:when test="$sequence = 'first' and $position='center'">
        <fo:block font-size="18pt" color="red" padding-before="0.13in">Podgląd struktury modułu e-podręcznika</fo:block>
      </xsl:when>
      ...
      <xsl:when test="$sequence = 'first' and $position='right'">
        <fo:external-graphic src="url({{LOGO}})" width="auto" height="auto" content-width="auto" content-height="50%"/>
      </xsl:when>
